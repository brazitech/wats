...
class KnowledgeHTMLWView extends WView {
    ...
    private function document() {
        // set the subtitle
        WDocumentHelper::subtitle(JText::_('WHD_KD:KNOWLEDGE'));

        WDocumentHelper::addPathwayItem(JText::_('WHD_KD:DOMAINS'), JText::_('WHD_KD:DOMAINS'), JRoute::_('index.php?option=com_whelpdesk&task=knowledgedomains.list.start'));
        WDocumentHelper::addPathwayItem($this->getModel('knowledgeDomain')->name, '', JRoute::_('index.php?option=com_whelpdesk&task=knowledgedomain.display&alias='.$this->getModel('knowledgeDomain')->alias));

        // add the current item to the end of the pathway
        WDocumentHelper::addPathwayItem(JText::_('WHD_KD:KNOWLEDGE'));
    }
}
