...
abstract class DocumentcontainerWController extends WController {
    ...
    public function commit($array) {
        ...
        WFactory::getOut()->log('Commited document container to the database');

        // add to the tree if necessary
        if (!array_key_exists('id', $array)) {
            try {
                $accessSession = WFactory::getAccessSession();
                $accessSession->addNode('documentcontainer', $table->id, $table->alias, 'documentcontainer', intval($array['parent']));
                WFactory::getOut()->log('Commited document container to the tree');
            } catch (Exception $e) {
                WFactory::getOut()->log('Failed to commit document container to the tree', $e->getMessage());
                return false;
            }
        }
